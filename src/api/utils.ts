...
import  type{ mem0RequestAdd, mem0RequestSearch, RetrivalRequest } from "../pipeline-processing/pipeline.js";
...
export const ingestion_helper = async (ingest:IngestionHelper) => {
    try{
        ...
        const _docIngest = await ingestDocument(bufferFile, ingest.filename, ingest.tags)
        if(!_docIngest) {
