...
import { SIGNED_URL_EXPIRES, ALLOWED_SIZE, ALLOWED_TYPES } from "../conf/const.ts";
...
export const createPresignedUrl = async (genKey:Key) => {
    try {
        ...
        if(!ALLOWED_TYPES.includes(genKey.contentType)) {
            ...
            return {TypeError: `Invalid File Type | Valid Type Are: ${ALLOWED_TYPES}` }
        }
        ...
        throw new StorageException(`Error occured while generating prsigned Url ${error}`)
    }
}
...
export const getFile = async (key:string) => {
    try {
        ...
        if(!response?.Body) {
            logger.error(`File is not present with following key ${key} in storage: ${response}`)
            throw new StorageException(`File is not present with following key ${key} in storage ${response}`)
        } 
        ...
        throw new StorageException(`Error occured while downloading file: ${error}`)
    }
}
