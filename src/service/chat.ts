  ...
  try {
    ...
    if (error) {
      ...
      throw new SuperBaseException(
        `Error while dumping the chat: ${error.message}`,
      );
    }
...
export const deleteChat = async (chat_id: string) => {
  try {
    ...
    if (error) {
      ...
      throw new SuperBaseException(
        `Error occured while deleting ${chat_id} : ${error.message}`,
      );
    }
    ...
    throw new SuperBaseException(
      `Error occured while deleting ${chat_id} : ${error}`,
    );
  }
};
...
export const deleteChats = async (session_id: string) => {
  try {
    ...
    if (error) {
      logger.error(
        `Error Occurred while deleting all sessions chat ${error.message}`,
      );
      return false;
    }
    ...
    throw new SuperBaseException(
      `Error occured while deleting all sessions chat ${error}`,
    );
  }
};
...
export const viewChats = async (view: ChatView) => {
  try {
    ...
    if (error) {
      logger.error(
        `Error Occured while fetching chat history : ${error.message}`,
      );
      throw new SuperBaseException(
        `Error Occured while fetching chat history : ${error.message}`,
      );
    ...
    throw new SuperBaseException(
      `Error Occured while fetching chat history : ${error}`,
    );
  }
};
