...
export const createIngestLog = async (logs: IngestLog) => {
  try {
    ...
    if (error) {
      logger.error(
        `Error occured while ingesting file for ${logs.session_id} :  ${error.message}`,
      );
      throw new SuperBaseException(
        `Error occured while ingesting file for for ${logs.session_id} : ${error.message}`,
      );
    }
...
export const updateIngestLog = async (logs: UpdateLog, ingestId: string) => {
  try {
    ...
    if (error) {
      logger.error(
        `Error occured while updating ingest file for ${ingestId} :  ${error.message}`,
      );
      throw new SuperBaseException(
